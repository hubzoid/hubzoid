...
def _raw_data_section(ctx: "HubContext") -> str:
    ...
    return "\n".join([
        ...
        "Before grepping, orient:",
        "",
        "1. `list_files('raw_data/*')` to see what is inside.",
        "2. `grep_data(pattern, path='raw_data/<scoped-folder>')` — always",
        "   scope `path` as narrowly as you can. Unscoped greps across",
        "   multiple repos waste tokens.",
        "3. `read_file(path, offset, limit)` for the specific file.",
        "",
