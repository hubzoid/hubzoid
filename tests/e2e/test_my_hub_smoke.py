"""End-to-end smoke test: real LLM call through the bridge against ../my-hub.

Requires OPENROUTER_API_KEY in the environment (or any provider key matching
the MODEL string in my-hub/.env). The test is marked `e2e` and skipped by
default; run with:
...
HUB = Path(__file__).resolve().parent.parent.parent / "my-hub"
...
def bridge_proc():
    if not HUB.is_dir():
        pytest.skip("my-hub/ not present")
